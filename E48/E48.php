...
<?php
    ...
    function ekle($gK) {
        try {
            global $sunucu, $vt, $ka, $sifre, $tablo;
            if (!empty($gK[0]) && !empty($gK[1]) && !empty($gK[2]) && !empty($gK[3])) {
                $bag = new PDO("mysql:host=$sunucu; dbname=$vt; charset=utf8", $ka, $sifre);
                ...
                if ($sorgu->rowCount() > 0) {
                    ...
                    echo "<script>alert('KAYIT EKLENEMEDİ. LÜTFEN TEKRAR DENEYİN!');</script>";
                }
            } else if (!empty($gK[0]) || !empty($gK[1]) || !empty($gK[2]) || !empty($gK[3])) {
                echo "<script>alert('LÜTFEN TÜM ALANLARI DOLDURUN!');</script>";
            }
        } catch (PDOException $hata) {
            echo "<script>alert('VERİTABANI HATASI: " . $hata->getMessage() . "');</script>";
        }
        $bag = null;
    }
...
<table style="border: 1px solid; width:100%">
    <tr>
        <td style="text-align: right; vertical-align: top; width:25%">
            <form action="E48.php" method="POST">
                İŞLEM:
